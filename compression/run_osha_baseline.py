...
def main():
    sep = "=" * 70
    print(sep); print("OSHA 1926 TRANSFER BASELINE -- v4.6 unchanged"); print(sep)
    ...
    Path(os.path.join(OSHA_DIR,"answers_osha_merged_baseline.json")).write_text(json.dumps(merged_results,indent=2))
    ...
    for i, q in enumerate(questions):
        ...
        print("    " + r["final_answer"][:100])
        if i<14: time.sleep(5)
    Path(os.path.join(OSHA_DIR,"answers_osha_split_baseline.json")).write_text(json.dumps(split_results,indent=2))
