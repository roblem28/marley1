#!/usr/bin/env python3
"""Relevance Filter v4.4 -- targeted patches for metering, islanding, antenna/RF.

v4.4 patches over v4.3:
A. Metering: stronger monitoring-specific ranking, deprioritize cybersecurity harder
B. Islanding: header boosts raised, behavioral line selection tightened
C. Antenna/RF: forced 2-aspect split (antenna entry + RF isolation)
D. Clipped-line filter: reject mid-sentence fragments
E. Regression protection on all v4.3 wins
"""
...
# ==================================================================
# QUERY FAMILY DETECTION (v4.4 -- added antenna_rf)
# ==================================================================
...
class RelevanceFilterV4:
    ...
    # ==================================================================
    # FAMILY FINALIZERS (v4.4 -- patched A, B, C)
    # ==================================================================

    def finalize_metering(self, evidence, query):
        """v4.4 Patch A: monitoring-specific ranking, hard deprioritize cybersecurity."""
        lines = [l.strip() for l in evidence.strip().split('\n') if l.strip()]
        lines, _ = _filter_clipped(lines)

        # Score each line for monitoring relevance
        scored = []
        for l in lines:
            ll = l.lower()
            s = 0.0
            if any(w in ll for w in ["remote monitoring", "monitoring and control", "control requirements",
                                      "metering requirements", "coordinated with the activity",
                                      "coordinate with the government", "coordinate with the activity"]): s += 3.0
            if re.search(r'comply\s+with\s+UFC\s+3-550', ll) or "exterior distribution" in ll: s += 1.5
            # Hard penalty for cybersecurity center of gravity
            if any(w in ll for w in ["ufc 4-010-06", "cybersecurity", "facility-related control systems"]):
                if not any(w in ll for w in ["remote monitoring", "monitoring and control", "metering"]): s -= 3.0
            scored.append((l, s))
        scored.sort(key=lambda x: x[1], reverse=True)

        # Build answer from top monitoring-specific lines
        monitoring_lines = [l for l, s in scored if s >= 1.5]
        dist_lines = [l for l, s in scored if 0 < s < 1.5]

        found = []
        if monitoring_lines:
            found.append(monitoring_lines[0])
        if dist_lines:
            found.append(dist_lines[0])
        if not found:
            # Fallback: best non-cyber requirement line
            req = [(l, s) for l, s in scored if s >= 0 and _REQUIREMENT_WORDS_RE.search(l)]
            if req:
                found.append(req[0][0])
    ...
    def finalize_islanding(self, evidence, query):
        """v4.4 Patch B: stronger behavioral line selection with header awareness."""
        lines = [l.strip() for l in evidence.strip().split('\n') if l.strip()]
        lines, _ = _filter_clipped(lines)
        ...
        for l in lines:
            ll = l.lower()
            s = 0.0
            # Header boosts (very high)
            if any(h in ll for h in ["utility interconnection and secure islanding",
                                      "restoration time and soft transition",
                                      "states of operation", "microgrid formation",
                                      "off-grid system endurance", "minimum design requirements"]): s += 4.0
            # Strong behavioral line boosts
            if any(w in ll for w in ["transition to islanded power", "transitioning to islanded power",
                                      "deliver power to designated critical",
                                      "responsible for transitioning"]): s += 2.5
            if any(w in ll for w in ["black start", "grid-forming"]): s += 2.5
            if any(w in ll for w in ["off-grid endurance", "off-grid system endurance"]): s += 2.5
            if any(w in ll for w in ["secure islanding", "critical loads", "critical mission",
                                      "reference frequency"]): s += 2.0
            if any(w in ll for w in ["soft transition", "restoration time"]): s += 2.0
            if any(w in ll for w in ["disparate source", "grid-forming der", "minimum of one"]): s += 1.5
            # Penalty for compliance-only
            if re.search(r'commercial\s+utility\s+relations', ll) and s < 2.0: s -= 2.5
            if re.search(r'ieee\s+1547|ul\s+1741', ll) and s < 2.0: s -= 2.5
        ...
        # Build max 4 behavioral bullets, covering distinct concepts
        concepts = {
            "transition": ["transition to islanded", "transitioning to islanded", "responsible for transitioning", "critical load", "deliver power"],
            "blackstart": ["black start", "grid-forming"],
            "endurance": ["off-grid endurance", "off-grid system endurance", "endurance not less"],
            "secure": ["secure islanding", "states of operation", "soft transition", "restoration time", "microgrid formation"],
        }
        kept = []
        used_concepts = set()
        for l, s in scored:
            ...
            if len(kept) >= 4: break
            ll = l.lower()
            # Which concept does this line serve?
    ...
    def finalize_c5isr_power_grounding(self, evidence, query, headers=None, chunks=None, top_indices=None):
        ...
        for l in lines:
            ll = l.lower()
            if any(w in ll for w in ["distribution architecture", "grade requirement", "grade",
                                      "black start", "backup power", "ups", "dc power", "power distribution"]):
                if l not in grounding: power.append(l)
        parts = []
        if power: parts.append("Power: " + ' '.join(power[:3]))
        if grounding: parts.append("Grounding and Bonding: " + ' '.join(grounding[:3]))
        if parts: return '\n'.join(parts)
        for l in lines:
...
if __name__ == "__main__":
    print("RelevanceFilterV4.4 loaded OK")
    for q, exp in [
