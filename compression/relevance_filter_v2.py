...
def detect_family(query):
    ...
    # CFR-generic families (domain-neutral, fire before OSHA-specific)
    has_cfr_cite = bool(re.search(r'\bd{1,3}\.d+', q))
    osha_specific = any(w in q for w in ["osha", "1926", "scaffold", "excavation", "crane", "hard hat", "competent person", "fall protection", "safety net", "confined space", "shoring", "trench", "construction site", "ladder", "subpart", "in construction"])
    if not osha_specific:
        if re.search(r'what\s+section\s+(?:governs|covers|requires|applies|addresses)', q): return "cfr_governing_reference"
        if re.search(r'(?:at\s+what|what\s+is\s+the|how\s+(?:much|many|fast|high))', q): return "cfr_numeric_requirement"
        if re.search(r'(?:minimum|maximum|threshold)\s+(?:altitude|airspeed|height|distance|width|depth|cabin\s+pressure|pressure)', q): return "cfr_numeric_requirement"
        if re.search(r'when\s+(?:is|must|should|are|does)', q): return "cfr_condition_triggered"
        if re.search(r'(?:exception|exempt|does\s+.*\s+apply|not\s+apply|unless|except)', q): return "cfr_exception_applicability"
        if re.search(r'what\s+(?:are\s+the\s+)?(?:required\s+)?(?:requirements|components|equipment|instruments|training|actions)', q): return "cfr_requirement_list"
    # OSHA / CFR families
...
def osha_rank_evidence(lines, family, query):
    ...
    q_nouns = set(re.findall(r'\b(?:scaffold|excavation|fall.protection|ladder|crane|hard.hat|fire.protection|confined.space|electrical|respiratory|safety.net|shoring|trench|guardrail|stairway|training|pilot|aircraft|altitude|airspeed|oxygen|seatbelt|preflight|flight.plan|ifr|vfr|ads.b|takeoff|landing|cabin|pic|alcohol|review|head.protection|pilot|aircraft|altitude|airspeed|oxygen|seatbelt|preflight|flight.plan|ifr|vfr|ads.b|takeoff|landing|cabin|pic|alcohol|review)\b', ql))
    ...
    for line in lines:
        ...
        if family in ("osha_governing", "cfr_governing_reference"):
            if _CFR_SECTION_RE.search(line): s += 3.0
            if re.search(r'\b(?:comply|accordance|refer|apply|governed)\b', ll): s += 2.0
        elif family in ("osha_numeric", "cfr_numeric_requirement"):
        ...
        elif family in ("osha_exception", "cfr_exception_applicability"):
            if _EXCEPTION_WORDS_RE.search(line): s += 5.0
        elif family in ("osha_condition", "cfr_condition_triggered"):
        ...
        elif family in ("osha_requirement_list", "cfr_requirement_list"):
...
class RelevanceFilterV4:
    ...
    def deterministic_answer(self, answer_mode, evidence, query, family=None,
        ...
        if family and (family.startswith("osha_") or family.startswith("cfr_")):
            ev_lower = evidence.lower()
            q_nouns = set(re.findall(r'\b(?:scaffold|excavation|fall.protection|ladder|crane|hard.hat|fire.protection|confined.space|electrical|respiratory|safety.net|shoring|trench|guardrail|stairway|training|pilot|aircraft|altitude|airspeed|oxygen|seatbelt|preflight|flight.plan|ifr|vfr|ads.b|takeoff|landing|cabin|pic|alcohol|review)\b', query.lower()))
            ev_noun_hits = sum(1 for n in q_nouns if n in ev_lower)
            cfr_refs = re.findall(r'(?:§\s*)?\d{1,3}\.\d+', evidence)
            ...
            if family in ("osha_governing","osha_numeric","osha_exception","osha_condition","cfr_governing_reference","cfr_numeric_requirement","cfr_exception_applicability","cfr_condition_triggered") and top_score < 2.5:
        ...
        if family and (family.startswith("osha_") or family.startswith("cfr_")) and evidence and evidence.strip():
