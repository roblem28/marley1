#!/usr/bin/env python3
"""Eval v4.7: hybrid-best build. answers_v4_7_quick.json."""
...
from v47_tournament import (run_tournament, frozen_passthrough, score_topic_purity,
                             score_contrastive, predict_shape, classify_actual_shape)
...
WEAK_FAMILIES = {"metering", "islanding", "closet_storage"}
...
def run_single(rf, case, dc, dh, de, db, dtk, dpt, dt):
    ...
    tourn = None
    if family in WEAK_FAMILIES and evidence and evidence.strip():
        tourn = run_tournament(family, rf, evidence, query, headers, chunks, ti)
    ...
    if "blast" in query.lower() and am=="governing_reference":
        ...
        # v4.7 TOURNAMENT OVERRIDE
        if tourn and tourn.get("winning_answer"):
            tw = tourn["winning_answer"]
            tw_tp = tourn.get("topic_purity_score", 0)
            base_tp = score_topic_purity(answer, family)
            if tw_tp >= base_tp:
                answer = tw
                det_stop = "tournament_" + str(tourn.get("winning_candidate_type", ""))
        frozen = family not in WEAK_FAMILIES
        if frozen:
            tourn = frozen_passthrough(answer, family)
    ...
    result = {
        ...
        "frozen_family_preserved": family not in WEAK_FAMILIES,
    }
    # Add tournament fields
    if tourn:
        for k in ["candidate_answers","candidate_scores","winning_candidate_type",
                   "proof_objects_used","contrastive_positive_hits","contrastive_negative_hits",
                   "contrastive_score","expected_answer_shape","actual_answer_shape",
                   "topic_purity_score","contamination_penalty","usefulness_score"]:
            result[k] = tourn.get(k, 0 if "score" in k or "hits" in k or "penalty" in k else None)
    else:
        result.update({"candidate_answers":None,"candidate_scores":None,"winning_candidate_type":None,
            "proof_objects_used":None,"contrastive_positive_hits":0,"contrastive_negative_hits":0,
            "contrastive_score":0,"expected_answer_shape":predict_shape(family),
            "actual_answer_shape":classify_actual_shape(answer),
            "topic_purity_score":score_topic_purity(answer,family or ""),
            "contamination_penalty":0,"usefulness_score":0})
    return result

def main():
    print(f"\n{'='*70}")
    print("EVAL V4.7 -- hybrid-best build (GO-FOR-BROKE)")
    print(f"{'='*70}")
    ...
    for i, case in enumerate(HARD_CASES):
        ...
        # Tournament fields already in r from run_single
        if i<len(HARD_CASES)-1: print("  Cooldown 3s..."); time.sleep(3)

    out=OUT_DIR/"answers_v4_7_quick.json"
    ...
    # Tournament checks
    if me: chk("Metering: tournament ran", me.get("winning_candidate_type") is not None)
    if isl: chk("Islanding: tournament ran", isl.get("winning_candidate_type") is not None)
    cs2=next((r for r in results if "closet" in r["query"].lower()),None)
    if cs2: chk("Closet: tournament ran", cs2.get("winning_candidate_type") is not None)
    chk("All frozen preserved", all(r.get("frozen_family_preserved",True) for r in results if r.get("family_name") not in WEAK_FAMILIES))
